...
import scda.service.UsuarioService;
import scda.util.MensagemUtil;
import scda.util.exception.BOException;
...
getListaSelecionarPerfil();
	...
	public String salvar() {
		...
		if (!senha.equals(this.confirmarSenha)) {
			MensagemUtil.addInfoMessage("A senha não foi confirmada corretamente");
			return null;
		}
		...
		if (this.destinoSalvar.equals("usuarioSucesso")) {
			try {
				...
				MensagemUtil.addErrorMessage("Não foi possível enviar o e-mail de cadastro do usuário. Erro: " + e.getMessage());
				return null;
			}
		}
	...
	public List<SelectItem> getListaSelecionarPerfil() {
		if (this.listaSelecionarPerfil == null) {
			...
			this.listaSelecionarPerfil.add(new SelectItem("ROLE_USER","Usuário"));
			this.listaSelecionarPerfil.add(new SelectItem("ROLE_PROJUR","Jurídico"));
